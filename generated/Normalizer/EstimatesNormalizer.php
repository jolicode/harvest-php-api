...
class EstimatesNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return $type === 'JoliCode\\Harvest\\Api\\Model\\Estimates';
    }
    public function supportsNormalization($data, $format = null)
    {
        return is_object($data) && get_class($data) === 'JoliCode\\Harvest\\Api\\Model\\Estimates';
    }
    public function denormalize($data, $class, $format = null, array $context = array())
    {
        ...
        $object = new \JoliCode\Harvest\Api\Model\Estimates();
        if (\array_key_exists('estimates', $data) && $data['estimates'] !== null) {
            $values = array();
            foreach ($data['estimates'] as $value) {
                $values[] = $this->denormalizer->denormalize($value, 'JoliCode\\Harvest\\Api\\Model\\Estimate', 'json', $context);
            }
            $object->setEstimates($values);
        }
        elseif (\array_key_exists('estimates', $data) && $data['estimates'] === null) {
            $object->setEstimates(null);
        }
    ...
    {
        $data = array();
        if (null !== $object->getEstimates()) {
            $values = array();
            foreach ($object->getEstimates() as $value) {
                $values[] = $this->normalizer->normalize($value, 'json', $context);
            }
            $data['estimates'] = $values;
        }
