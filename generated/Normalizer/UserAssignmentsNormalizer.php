...
class UserAssignmentsNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return \JoliCode\Harvest\Api\Model\UserAssignments::class === $type;
    }

    public function supportsNormalization(mixed $data, ?string $format = null, array $context = []): bool
    {
        return \is_object($data) && \JoliCode\Harvest\Api\Model\UserAssignments::class === $data::class;
    }
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('user_assignments', $data)) {
            $values = [];
            foreach ($data['user_assignments'] as $value) {
                $values[] = $this->denormalizer->denormalize($value, \JoliCode\Harvest\Api\Model\UserAssignment::class, 'json', $context);
            }
            $object->setUserAssignments($values);
            unset($data['user_assignments']);
        }
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        $dataArray = [];
        $values = [];
        foreach ($data->getUserAssignments() as $value) {
            $values[] = $this->normalizer->normalize($value, 'json', $context);
        }
        $dataArray['user_assignments'] = $values;
    ...
    {
        return [\JoliCode\Harvest\Api\Model\UserAssignments::class => false];
    }
}
