...
class CostRatesNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    use DenormalizerAwareTrait;
    use NormalizerAwareTrait;
    public function supportsDenormalization($data, $type, $format = null)
    {
        return $type === 'JoliCode\\Harvest\\Api\\Model\\CostRates';
    }
    public function supportsNormalization($data, $format = null)
    {
        return is_object($data) && get_class($data) === 'JoliCode\\Harvest\\Api\\Model\\CostRates';
    }
    public function denormalize($data, $class, $format = null, array $context = array())
    {
        ...
        $object = new \JoliCode\Harvest\Api\Model\CostRates();
        if (property_exists($data, 'cost_rates') && $data->{'cost_rates'} !== null) {
            $values = array();
            foreach ($data->{'cost_rates'} as $value) {
                $values[] = $this->denormalizer->denormalize($value, 'JoliCode\\Harvest\\Api\\Model\\CostRate', 'json', $context);
            }
            $object->setCostRates($values);
        }
        elseif (property_exists($data, 'cost_rates') && $data->{'cost_rates'} === null) {
            $object->setCostRates(null);
        }
    ...
    {
        $data = new \stdClass();
        if (null !== $object->getCostRates()) {
            $values = array();
            foreach ($object->getCostRates() as $value) {
                $values[] = $this->normalizer->normalize($value, 'json', $context);
            }
            $data->{'cost_rates'} = $values;
        }
        else {
            $data->{'cost_rates'} = null;
        }
