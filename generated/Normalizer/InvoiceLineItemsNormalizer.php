...
class InvoiceLineItemsNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    use DenormalizerAwareTrait;
    use NormalizerAwareTrait;
    public function supportsDenormalization($data, $type, $format = null)
    {
        return $type === 'JoliCode\\Harvest\\Api\\Model\\InvoiceLineItems';
    }
    public function supportsNormalization($data, $format = null)
    {
        return is_object($data) && get_class($data) === 'JoliCode\\Harvest\\Api\\Model\\InvoiceLineItems';
    }
    public function denormalize($data, $class, $format = null, array $context = array())
    {
        ...
        $object = new \JoliCode\Harvest\Api\Model\InvoiceLineItems();
        if (property_exists($data, 'invoice_line_items') && $data->{'invoice_line_items'} !== null) {
            $values = array();
            foreach ($data->{'invoice_line_items'} as $value) {
                $values[] = $this->denormalizer->denormalize($value, 'JoliCode\\Harvest\\Api\\Model\\InvoiceLineItem', 'json', $context);
            }
            $object->setInvoiceLineItems($values);
        }
        elseif (property_exists($data, 'invoice_line_items') && $data->{'invoice_line_items'} === null) {
            $object->setInvoiceLineItems(null);
        }
    ...
    {
        $data = new \stdClass();
        if (null !== $object->getInvoiceLineItems()) {
            $values = array();
            foreach ($object->getInvoiceLineItems() as $value) {
                $values[] = $this->normalizer->normalize($value, 'json', $context);
            }
            $data->{'invoice_line_items'} = $values;
        }
        else {
            $data->{'invoice_line_items'} = null;
        }
